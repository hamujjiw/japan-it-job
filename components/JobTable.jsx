...
const GATE_DOT_LABEL = {
  bahasa: { lolos: "Bahasa: lolos", sebagian: "Bahasa: sebagian", terkunci: "Bahasa: terkunci", "tidak-jelas": "Bahasa: tidak jelas" },
  lamaran: { lolos: "Lamaran: lolos", sebagian: "Lamaran: sebagian", terkunci: "Lamaran: terkunci", "tidak-jelas": "Lamaran: tidak jelas" },
  lokasi: { lolos: "Lokasi: lolos", sebagian: "Lokasi: sebagian", terkunci: "Lokasi: terkunci", "tidak-jelas": "Lokasi: tidak jelas" },
};

/**
 * Tampilan tabel. Dipakai saat orang mau bandingkan banyak lowongan
 * sekaligus (gaji, gerbang, kota) dalam satu pandangan, tanpa buka satu-satu
          ...
          {jobs.map((job) => {
            ...
            return (
              <tr key={job.id} onClick={() => onOpen(job)} tabIndex={0}
                ...
                <td>
                  <div className="mini-gates" role="group" aria-label="Kelayakan">
                    {["bahasa", "lamaran", "lokasi"].map((k) => (
                      <span
                        key={k}
                        className="mini-dot"
                        data-state={job.gates[k]}
                        title={GATE_DOT_LABEL[k][job.gates[k]]}
                      />
                    ))}
                  </div>
                </td>
